...
public class FolderServiceImplTest {
	...
	public void testUpdateFolder_movingFolderNoMessagesNoNestedFolders()
		...
		assertEquals("INBOX.target", uut.findById("INBOX.target.folder")
				.getParentFolderId());
	}
	...
	public void testUpdateFolder_movingFolderNoMessagesWithNestedFolders()
		...
		assertEquals("INBOX.target", uut.findById("INBOX.target.folder")
				.getParentFolderId());
		assertEquals(2, uut.findByParent("INBOX.target.folder").getFolderList()
				.size());
	}
	...
	public void testUpdateFolder_movingFolderWithMessagesWithNestedFolders()
			throws InterruptedException {
		mockServer.createAndSubscribeMailBox("INBOX.source");
		mockServer.prepareMailBox("INBOX.source.folder",
				new MimeMessageBuilder().build(3));
		mockServer.prepareMailBox("INBOX.source.folder.nested1",
				new MimeMessageBuilder().build(4));
		mockServer.prepareMailBox("INBOX.source.folder.nested2",
				new MimeMessageBuilder().build(5));
		mockServer.createAndSubscribeMailBox("INBOX.target");
	...
	public void testDeleteFolder_trashedWithMessagesWithNestedFolders() {
		mockServer.createAndSubscribeMailBox("INBOX.Trash.deleteme");
		mockServer.prepareMailBox("INBOX.Trash.deleteme",
				new MimeMessageBuilder().build(3));
		mockServer.prepareMailBox("INBOX.Trash.deleteme.nested1",
				new MimeMessageBuilder().build(4));
		mockServer.prepareMailBox("INBOX.Trash.deleteme.nested2",
				new MimeMessageBuilder().build(5));

		uut.deleteFolder("INBOX.Trash.deleteme.nested1");

		assertEquals(1, uut.findByParent("INBOX.Trash").getFolderList().size());
		assertEquals(1, uut.findByParent("INBOX.Trash.deleteme")
				.getFolderList().size());
	...
	public void testDeleteFolder_noMessagesNoNestedFolders() {
		...
		assertEquals("INBOX.Trash.deleteme2",
				uut.findById("INBOX.Trash.deleteme2").getId());
	}

	@Test
	public void testDeleteFolder_noMessagesWithNestedFolders() {
		mockServer.prepareMailBox("INBOX.deleteme2",
				new MimeMessageBuilder().build(2));
	...
	public void testDeleteFolder_withMessagesWithNestedFolders() {
		...
		mockServer.prepareMailBox("INBOX.test.deleteme2",
				new MimeMessageBuilder().build(3));
		mockServer.prepareMailBox("INBOX.test.deleteme2.nested1",
				new MimeMessageBuilder().build(4));
		mockServer.prepareMailBox("INBOX.test.deleteme2.nested2",
				new MimeMessageBuilder().build(5));

		uut.deleteFolder("INBOX.test.deleteme2.nested1");

		assertEquals(1, uut.findByParent("INBOX.Trash").getFolderList().size());
		assertEquals(1, uut.findByParent("INBOX.test.deleteme2")
				.getFolderList().size());
