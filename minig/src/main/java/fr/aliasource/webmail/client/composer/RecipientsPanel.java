...
public class RecipientsPanel extends HorizontalPanel {
                ...
                if (mailField.getText().isEmpty()) {
                    ...
                    case KeyCodes.KEY_BACKSPACE:
                        if (recipCount > 0) {
                            String email = "";
                            if (token == -1) {
                                email = recipListPanel.getWidget(recipCount - 1).getTitle();
                                recipListPanel.getWidget(recipCount - 1).removeFromParent();
                            } else {
                                email = recipListPanel.getWidget(token).getTitle();
                                recipListPanel.getWidget(token).removeFromParent();
                            }
                            recipients.remove(email);
                            mailField.setFocus(true);
                            token = -1;
                    ...
                    case KeyCodes.KEY_LEFT:
                        if (recipCount > 0) {
                            ...
                            recipListPanel.getWidget(token).addStyleName("highlightRecipient");
                        }
                        break;
                    ...
                    case KeyCodes.KEY_DELETE:
                        if (token >= 0) {
                            String email = recipListPanel.getWidget(token).getTitle();
    ...
    public void setRecipients(List<IEmailAddress> ads) {
        clearText();
        if (ads != null) {
            for (IEmailAddress addr : ads) {
    ...
    public List<IEmailAddress> getRecipients() {
        ArrayList<IEmailAddress> ad = new ArrayList<IEmailAddress>(recipients.size());
