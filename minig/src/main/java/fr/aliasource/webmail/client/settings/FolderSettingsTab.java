...
public class FolderSettingsTab extends DockPanel {
    ...
    private void createFolder() {
        Ajax<ICreateFolderRequest> builder = AjaxFactory.createFolder(dataGrid.getCurrentPath());
        ICreateFolderRequest createFolderRequest = BeanFactory.instance.createFolder().as();

        createFolderRequest.setFolder(folderName.getText());

        try {
            builder.send(createFolderRequest, new AjaxCallback<ICreateFolderRequest>() {
    ...
    public void selectFolder(IFolder folder) {
        if (folder != null) {
            ...
            createLabel.setHTML(I18N.strings.createSubFolderIn() + " <b>" + folder.getId() + "</b>: ");
            folderName.setText("");
